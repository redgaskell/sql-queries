...
FROM invoices WHERE billing_country = 'USA'
...
SELECT billing_city, SUM(total) AS invoice_count
FROM invoices WHERE billing_country = 'USA'
GROUP BY billing_city 
ORDER BY invoice_count DESC
 LIMIT 10;
...
SELECT billing_city, AVG(total) AS invoice_count
FROM invoices WHERE billing_country = 'USA'
GROUP BY billing_city 
ORDER BY invoice_count DESC
 LIMIT 10;
...
SELECT billing_city, COUNT(*) AS invoice_count
FROM invoices WHERE billing_country = 'USA' AND billing_state = 'CA'
GROUP BY billing_city 
ORDER BY invoice_count DESC
 LIMIT 3;
